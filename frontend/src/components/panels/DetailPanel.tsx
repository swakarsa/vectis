...
export function DetailPanel({
  ...
  const copyShim = () => {
    if (navigator?.clipboard?.writeText) {
      navigator.clipboard.writeText(shimCode).catch(() => {});
    }
    setCopied(true);
  ...
  return (
    <aside className="w-full lg:w-[420px] border-l border-white/[0.08] bg-[#0c0d10] flex flex-col h-full select-none shrink-0 overflow-hidden">
      ...
      <div className="flex border-b border-white/[0.08] bg-[#090a0d] px-1 text-xs">
        ...
        >
          <Scales size={13} className="shrink-0 text-zinc-300" />
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 font-semibold text-rose-400">
                      ...
                      <span className="text-[9px] px-1 py-0.5 rounded-[2px] bg-rose-500/20 text-rose-300 border border-rose-500/30 uppercase tracking-wide font-sans">
                        Critical
                      </span>
                    </div>
                  ...
                  <div className="border border-white/[0.05] rounded-[3px] p-2 bg-[#090a0d] space-y-1">
                    <div className="flex items-center gap-2 text-[11px]">
                      ...
                      <span className="text-zinc-300 truncate tabular-nums font-sans text-[10px]" title={change.old_signature}>{change.old_signature}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px]">
                      <span className="text-emerald-400 shrink-0 font-medium text-[10px]">NEW:</span>
                      <span className="text-zinc-300 truncate tabular-nums font-sans text-[10px]" title={change.new_signature}>{change.new_signature}</span>
                    </div>
                  </div>
                </div>
              ))
            ...
            {isIdle ? (
              <div className="flex flex-col items-center justify-center py-14 px-4 text-center">
                ...
                <p className="text-[11px] text-zinc-500 max-w-[240px] leading-relaxed">
                  Run Gate Audit or Inject Hazard to trigger autonomous IBM Granite 3.0 code synthesis and contract healing.
                </p>
              </div>
            ) : verdict === "PASS" && !shimApplied ? (
              <div className="flex flex-col items-center justify-center py-14 px-4 text-center">
                <div className="w-9 h-9 rounded-[6px] bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mb-2.5">
                  <ShieldCheck size={18} />
                </div>
                ...
                <p className="text-[11px] text-zinc-400 max-w-[240px] leading-relaxed">
                  Zero breaking mutations detected across interface definitions. No backward-compatibility shims required.
                </p>
              </div>
            ) : (
              <>
          ...
          <div className="space-y-3">
            {isIdle ? (
              <div className="flex flex-col items-center justify-center py-14 px-4 text-center">
                ...
                <div className="text-xs font-medium text-zinc-300 mb-1">
                  Regulatory Audit Standby
                </div>
                <p className="text-[11px] text-zinc-500 max-w-[240px] leading-relaxed">
                  Docling engine is ready to extract and verify compliance mandates against live changesets. Run an audit to verify.
                </p>
              </div>
            ) : (
              <>
